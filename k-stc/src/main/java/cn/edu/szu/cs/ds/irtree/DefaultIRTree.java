 ...
 */
public final class DefaultIRTree implements IRTree<DefaultRelatedObject> {

    /**
     * rTree
     */
    private final RTree<DefaultRelatedObject, GeoPointDouble> rTree;
    /**
     * Record the relationship between non-leaf nodes and IF
     */
    private final Map<Node<DefaultRelatedObject, GeoPointDouble>, Map<String, List<IRtreeNode>>> nodeInvertedIndexMap;

    private final Map<Node<DefaultRelatedObject, GeoPointDouble>, Map<String, List<IRtreeLeafNode>>> leafNodeInvertedIndexMap;

    private static final LFUCache<String, List<Entry<DefaultRelatedObject, GeoPointDouble>>> cache = CacheUtil.newLFUCache(100);

    @Data
    private static class IRtreeNode {
        private Node<DefaultRelatedObject, GeoPointDouble> node;
        private Double weight;

        public IRtreeNode(Node<DefaultRelatedObject, GeoPointDouble> node, Double weight) {
            this.node = node;
            this.weight = weight;
        }
    ...
    private static class IRtreeLeafNode {
        private Entry<DefaultRelatedObject, GeoPointDouble> entry;
        private Double weight;

        public IRtreeLeafNode(Entry<DefaultRelatedObject, GeoPointDouble> entry, Double weight) {
            this.entry = entry;
            this.weight = weight;
        }
    ...
    public DefaultIRTree(RTree<DefaultRelatedObject, GeoPointDouble> rTree) {
    ...
    private Map<String, Double> buildIRtree(Node<DefaultRelatedObject, GeoPointDouble> curNode) {

        // Processing leaf nodes
        if (curNode instanceof LeafDefault) {
            LeafDefault<DefaultRelatedObject, GeoPointDouble> leaf = (LeafDefault) curNode;

            Map<String, List<IRtreeLeafNode>> iTreeLeafNodeMap = new HashMap<>();

            List<Entry<DefaultRelatedObject, GeoPointDouble>> entries = leaf.entries();

            Map<String, Double> ans = new HashMap<>();
            for (Entry<DefaultRelatedObject, GeoPointDouble> entry : entries) {

                DefaultRelatedObject relatedObject = entry.value();
        ...
        if (curNode instanceof NonLeafDefault) {
            NonLeafDefault<DefaultRelatedObject, GeoPointDouble> nonLeaf = (NonLeafDefault<DefaultRelatedObject, GeoPointDouble>) curNode;

            Map<String, List<IRtreeNode>> nodeIf = new HashMap<>(nonLeaf.count());

            Map<String, Double> ans = new HashMap<>();
            for (Object child : nonLeaf.children()) {
                Node<DefaultRelatedObject, GeoPointDouble> childNode = (Node<DefaultRelatedObject, GeoPointDouble>) child;
    ...
    private List<IRtreeNode> getNodesByKeyword(Node<DefaultRelatedObject, GeoPointDouble> curNode, String keyword) {
    ...
    private Set<IRtreeNode> getNodesByKeywords(Node<DefaultRelatedObject,GeoPointDouble> curNode, List<String> keywords) {
    ...
    private List<Entry<DefaultRelatedObject, GeoPointDouble>> filterByKeywords(List<String> keywords) {
        ...
        Node<DefaultRelatedObject, GeoPointDouble> root = rTree.root().get();

        Deque<Node<DefaultRelatedObject, GeoPointDouble>> que = new ArrayDeque<>();
        que.add(root);
        while (!que.isEmpty()) {
            Node<DefaultRelatedObject, GeoPointDouble> node = que.peek();
            ...
            if (node instanceof NonLeafDefault) {
                ...
                for (int i = 0; i < size; i++) {
                    Node<DefaultRelatedObject, GeoPointDouble> curNode = que.poll();
        ...
        List<Entry<DefaultRelatedObject, GeoPointDouble>> entries = que.stream()
                .map(node -> ((LeafDefault<DefaultRelatedObject, GeoPointDouble>) node).entries())
    ...
    @Override
    public synchronized List<DefaultRelatedObject> rangeQuery(List<String> keywords, double[] coordinate, double epsilon) {
        ...
        List<Entry<DefaultRelatedObject, GeoPointDouble>> entries = filterByKeywords(keywords);
