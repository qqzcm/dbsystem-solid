...
import cn.edu.szu.cs.kstc.optics.OpticsOg;
...
public class OpticsOgApproachDataFetchAction implements DataFetchAction<KstcQuery, List> {

    private Log log = LogFactory.get();

    private static final String CACHE_KEY_PREFIX = "OPTICS_OG_BASED_APPROACH:{0}";
    private LRUCache<String,List> cache = CacheUtil.newLRUCache(128, 3600 * 1000L);

    private OpticsOg opticsBasedApproach;

    public OpticsOgApproachDataFetchAction() {
        this.opticsBasedApproach = new OpticsOg();
    }

    @Override
    public String getCommand() {
        return DataFetchConstant.OPTICS_BASED_APPROACH_OG;
    }
