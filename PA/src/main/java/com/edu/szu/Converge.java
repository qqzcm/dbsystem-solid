...
public class Converge {
    ...
    private static void preComputeParameter(String input_path, String splitStr) throws Exception {
        ...
        CORE_COLUMN_LENGTH = globalSize.length - 3;
    ...
    public static void findSubTensor(List<Integer> globalList, PriorityQueue<Pair<Tensor, Double>> subTensorPriorityQueue, int k, int circleNum) throws CloneNotSupportedException {
        while(true){
            ...
            // int[][] dp1 = new int[3][globalMaxTime + 2];
            // for(int i = 1; i <= globalMaxTime; i++){
            //     dp1[0][i] = Math.max(stMass[0][i], dp1[0][i - 1]);
            //     dp1[1][globalMaxTime - i + 1] = Math.max(stMass[0][globalMaxTime - i + 1], dp1[1][globalMaxTime - i + 2]);
            // }
            // for(int i = 2; i <= globalMaxTime - 1; i++){
            //     dp1[2][i] = min(dp1[0][i], dp1[1][i]) - stMass[0][i];
            //     if(dp1[2][i] > maxGap[0]){
            //         maxGap[0] = dp1[2][i];
            //         maxIndex[0] = i;
            //     }
            ...
            int[][] dp2 = new int[3][globalMaxLatitude + 2];
            for(int i = 1; i <= globalMaxLatitude; i++){
                dp2[0][i] = Math.max(stMass[1][i], dp2[0][i - 1]);
                dp2[1][globalMaxLatitude - i + 1] = Math.max(stMass[1][globalMaxLatitude - i + 1], dp2[1][globalMaxLatitude - i + 2]);
            }
            for(int i = 2; i <= globalMaxLatitude - 1; i++){
                dp2[2][i] = min(dp2[0][i], dp2[1][i]) - stMass[1][i];
                if(dp2[2][i] > maxGap[1]){
                    maxGap[1] = dp2[2][i];
                    maxIndex[1] = i;
                }
            ...
            int[][] dp3 = new int[3][globalMaxlongitude + 2];
            for(int i = 1; i <= globalMaxlongitude; i++){
                dp3[0][i] = Math.max(stMass[2][i], dp3[0][i - 1]);
                dp3[1][globalMaxlongitude - i + 1] = Math.max(stMass[2][globalMaxlongitude - i + 1], dp3[1][globalMaxlongitude - i + 2]);
            }
            for(int i = 2; i <= globalMaxlongitude - 1; i++){
                dp3[2][i] = min(dp3[0][i], dp3[1][i]) - stMass[2][i];
                if(dp3[2][i] > maxGap[2]){
                    maxGap[2] = dp3[2][i];
                    maxIndex[2] = i;
                }
            ...
            //用于删除时判断方向
            for (Integer j : globalList) {
                int tempAttr = attVals[mode][j];
                if (tempAttr > attr) {
                    dataList2.add(j);
                } else {
                    dataList1.add(j);
            ...
            //获取划分后的两个子张量密度
            long start = System.currentTimeMillis();
            double[] measurementAfter = tensor.preDivideSlice(dataList1, dataList2);
            Converge.demoTime3 += System.currentTimeMillis() - start;
            //P_I
            ...
            if(measurementAfter[0] >= measurementBefore || measurementAfter[1] >= measurementBefore){
                long startTime = System.currentTimeMillis();
                // 开始划分
                if(measurementAfter[0] > measurementAfter[1]){
                    globalList = dataList1;
                }else{
                    globalList = dataList2;
                }
                demoTime4 += System.currentTimeMillis() - startTime;
                // if(subTensor1 != null){
                // }

                // tensorQueue.add(subTensor1);
                // tensorQueue.add(subTensor2);
            }/*else if(Util.preDeleteSlice(attr, mode, globalList) > measurementBefore){
                long startTime = System.currentTimeMillis();
                Util.deleteSlice(attr, mode, globalList);
                demoTime3 += System.currentTimeMillis() - startTime;
            }*/else{
                break;
            }
        }
    ...
    public static void processCoreDimension(Tensor subTensor, int k, PriorityQueue<Pair<Tensor, Double>> subTensorPriorityQueue, int circleNum) throws CloneNotSupportedException {
        ...
        if(spaceRange <= spacePropogation){
