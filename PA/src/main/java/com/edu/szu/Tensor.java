...
public class Tensor implements Cloneable{
    ...
    public void updatePeakInfomation() {
        ...
        for (int i = 0; i < value.length; i++) {
            if(value[i] != 0){
                maxTime = Math.max(Converge.attVals[Converge.TIME_COLUMN][i], maxTime);
                minTime = Math.min(Converge.attVals[Converge.TIME_COLUMN][i], minTime);
                maxLatitude = Math.max(Converge.attVals[Converge.LATITUDE_COLUMN][i], maxLatitude);
                minLatitude = Math.min(Converge.attVals[Converge.LATITUDE_COLUMN][i], minLatitude);
                maxLongitude = Math.max(Converge.attVals[Converge.LONGITUDE_COLUMN][i], maxLongitude);
                minLongitude = Math.min(Converge.attVals[Converge.LONGITUDE_COLUMN][i], minLongitude);
            }
        }
    }
    ...
    public String toString() {
        return "Tensor{" +
                ...
                ", attVals=" + Arrays.toString(Converge.attVals) +
    ...
    public Tensor(int minTime, int maxTime, int minLatitude, int maxLatitude, int minLongitude, int maxLongitude, int mass, int[] size,  int[] value, List<List<Slice>> slices, List<Map<Integer, Integer>> dataOfAttr) {
    ...
    public Tensor createSubTensorBydataList(List<Integer> dataList){
        ...
        for (int j = 0; j < Converge.TIME_COLUMN; j++) {
        ...
        for (Integer data : dataList) {
            ...
            for (int i = 0; i < Converge.CORE_COLUMN_LENGTH; i++) {
                ...
                Integer tempValue = datas.get(Converge.attVals[i][data]);
                if(tempValue == null){
                    datas.put(Converge.attVals[i][data], 0);
                    tempValue = 0;
                    newSize[i]++;
                }
                datas.replace(Converge.attVals[i][data], tempValue + value[data]);

            }

            //处理时间的dataOfAttr
            Map<Integer, Integer> datas3 = newDataOfAttr.get(Converge.TIME_COLUMN);
            Integer tempValue = datas3.get(Converge.attVals[Converge.TIME_COLUMN][data]);
            if(tempValue == null){
                datas3.put(Converge.attVals[Converge.TIME_COLUMN][data], 0);
                tempValue = 0;
                newSize[Converge.TIME_COLUMN]++;
            }
            datas3.replace(Converge.attVals[Converge.TIME_COLUMN][data], tempValue + value[data]);
            ...
            //使经度对应的keyValue自增1
            int latitude = Converge.attVals[Converge.LATITUDE_COLUMN][data];
            int longitude = Converge.attVals[Converge.LONGITUDE_COLUMN][data];
            ...
            for (int mode = 0; mode < Converge.globalSize.length - 1; mode++) {
                ...
                if(mode < Converge.TIME_COLUMN){
                    attr = Converge.attVals[mode][data];
                ...
                if(mode == Converge.TIME_COLUMN){
                    minTime = Math.min(minTime, Converge.attVals[mode][data]);
                    maxTime = Math.max(maxTime, Converge.attVals[mode][data]);
                }else if(mode == Converge.LATITUDE_COLUMN){
                    minLatitude = Math.min(minLatitude, Converge.attVals[Converge.LATITUDE_COLUMN][data]);
                    maxLatitude = Math.max(maxLatitude, Converge.attVals[Converge.LATITUDE_COLUMN][data]);
                }else if(mode == Converge.LONGITUDE_COLUMN){
                    minLongitude = Math.min(minLongitude, Converge.attVals[Converge.LONGITUDE_COLUMN][data]);
                    maxLongitude = Math.max(maxLongitude, Converge.attVals[Converge.LONGITUDE_COLUMN][data]);
                }
        ...
        return new Tensor(minTime, maxTime, minLatitude, maxLatitude, minLongitude, maxLongitude,newMass, newSize,  newValue, newSlices, newDataOfAttr);
    }
    ...
    public boolean preDeleteSlice(int[][] sliceInformation, int index) {
        ...
        for (Integer data : dataIndexs) {
            ...
            for(int i = 0; i < Converge.globalSize.length - 1; i++){
                int tempAttr = Converge.attVals[i][data];
                ...
                if(i == Converge.TIME_COLUMN){
                    if(Converge.attVals[i][data] == tempMinTime){
                    ...
                    }else if(Converge.attVals[i][data] == tempMaxTime){
                    ...
                    if(Converge.attVals[i][data] == tempMinLatitude){
                    ...
                    }else if(Converge.attVals[i][data] == tempMaxLatitude){
                    ...
                    if(Converge.attVals[i][data] == tempMinLongitude){
                    ...
                    }else if(Converge.attVals[i][data] == tempMaxLongitude){
        ...
        double measurementBefore = Util.computeMeasurement(densityBefore,  spatialRangeBefore);
        double measurementAfter = Util.computeMeasurement(densityAfter,  spatialRangeAfter);
    ...
    class AttrComparator implements Comparator<Integer>{
        ...
        public int compare(Integer o1, Integer o2) {
            if(value[o1] == 0 && value[o2] != 0){
                ...
                return Double.compare(Converge.attVals[Converge.LATITUDE_COLUMN][o1], Converge.attVals[Converge.LATITUDE_COLUMN][o2]);
            }
        }
    }
    ...
    public void deleteSlice(int[][] sliceInformation, int index) {
        ...
        for (int k = 0; k < dataIndexs.size(); k++) {
            ...
            for(int i = 0; i < Converge.globalSize.length - 1; i++){
                ...
                //更新最大最小时间信息
                if(i == Converge.TIME_COLUMN){
                    if(Converge.attVals[i][data] == minTime){
                    ...
                    }else if(Converge.attVals[i][data] == maxTime){
                    ...
                    //更新最小最大经纬度信息
                    if(Converge.attVals[i][data] == minLatitude){
                        for(int latitude = minLatitude; latitude <= maxLatitude; latitude++){
                    ...
                    }else if(Converge.attVals[i][data] == maxLatitude){
                        for(int latitude = maxLatitude; latitude >= minLatitude; latitude--){
                            if(slices.get(i).get(latitude - 1).isExists()){
                    ...
                    if(Converge.attVals[i][data] == minLongitude){
                        for(int longitude = minLongitude; longitude < maxLongitude; longitude++){
                    ...
                    }else if(Converge.attVals[i][data] == maxLongitude){
                        for(int longitude = maxLongitude; longitude >= minLongitude; longitude--){
                            if(slices.get(i).get(longitude - 1).isExists()){
     ...
     * @return
     */
    public double[] preDivideSlice(List<Integer> dataList1, List<Integer> dataList2) {
        ...
        for (int i = 0; i < dataList1.size(); i++) {
            ...
            for (int j = 0; j < Converge.globalSize.length - 1; j++) {
                totalSizeList1.get(j).add(Converge.attVals[j][dataList1.get(i)]);
            }
        }
        for (int i = 0; i < dataList2.size(); i++) {
            totalMass2 += value[dataList2.get(i)];
            for (int j = 0; j < Converge.globalSize.length - 1; j++) {
                totalSizeList2.get(j).add(Converge.attVals[j][dataList2.get(i)]);
            }
        }
        ...
        result[0] = Util.computeMeasurement(densityAfter1,  spatialRangeAfter1);
        result[1] = Util.computeMeasurement(densityAfter2,  spatialRangeAfter2);
