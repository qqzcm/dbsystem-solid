...
void NKDV_algorithm(model& our_model)
{
	...
	// Branch based on kdv_type
	if (our_model.kdv_type == 3)
	{
		// Spatiotemporal mode: sort points by time first
		cout << "Running spatiotemporal NKDV (kdv_type=3)" << endl;
		sort_points_by_time(our_model);
	}
	else
	{
		cout << "Running spatial NKDV (kdv_type=1)" << endl;
	}
	...
	if (our_model.method == 1) //better version of NKDV baseline method
	{
		for (int l = 0; l < (int)our_model.lixel_set.size(); l++)
		{
			...
			// Call appropriate function based on kdv_type
			if (our_model.kdv_type == 3)
				NKDV_spatiotemporal(our_model);
			else
	...
	if (our_model.method >= 2 && our_model.method <= 5) //Save the number of calls in dijstra algorithm
	{
		...
		for (int l = 0; l < (int)our_model.lixel_set.size(); l++)
		{
			...
			if (edge_index != prev_edge_index)
			{
				...
				auto end_SP_time = chrono::high_resolution_clock::now();
			#endif
			// cout<< "lixel " << l << ", edge_index " << edge_index << endl;
