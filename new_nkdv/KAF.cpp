...
{
	// Temporal kernel function (default: Epanechnikov)
	double sq_time_dist = time_dist * time_dist;
	
	if (our_model.k_type_t == 0) //Gaussian kernel
		return exp(-our_model.gamma_t * sq_time_dist);
	
	//Triangular, Epanechnikov and Quartic kernels
	if (our_model.k_type_t >= 1 && our_model.k_type_t <= 3)
	{
		if (time_dist >= our_model.bandwidth_t)
			return 0;
		
		if (our_model.k_type_t == 1) //Triangular kernel
			return (1 - our_model.gamma_t * time_dist);
		if (our_model.k_type_t == 2) //Epanechnikov kernel
			return (1 - our_model.gamma_t * sq_time_dist);
		if (our_model.k_type_t == 3) //Quartic kernel
			return (1 - our_model.gamma_t * sq_time_dist) * (1 - our_model.gamma_t * sq_time_dist);
	}
...
{
	// Check if point is within [cur_time - bandwidth_t, cur_time + bandwidth_t]
	double time_dist = fabs(point_time - cur_time);
	return time_dist <= our_model.bandwidth_t;
}
...
void augment_preprocess(model& our_model)
{
	...
	// Note: In spatiotemporal mode (kdv_type=3), this preprocessing still computes
	// spatial aggregations. Time filtering will be applied during query time in edge_KAF_spatiotemporal.
	// The precomputed aggregations remain valid because they are based on spatial distances only.
...
// Spatiotemporal KDE calculation
...
double edge_KAF_spatiotemporal(model& our_model, int edge_index, double cur_time)
{
	...
	// ADA method with time window filtering (method 3)
	if (our_model.method == 3)
	{
		// For spatiotemporal mode with ADA, we need to iterate through points
		// and apply time window filtering before using spatial aggregations.
		// Note: This is a simplified implementation that filters points by time
		// and then applies spatial kernel calculations. A more optimized version
		// could precompute time-windowed aggregations, but that would require
		...
		if (cur_l.edge_index == edge_index)
		{
			...
			{
				// Check if point is in time window
				if (!is_point_in_time_window(our_model, PS[i].time, cur_time))
					continue;
				...
				time_dist = fabs(PS[i].time - cur_time);
				
				// Combine spatial and temporal kernels
				double spatial_kernel = kernel_value(our_model, dist_q_p);
				double temporal_kernel = kernel_value_temporal(our_model, time_dist);
				
				edge_KA_value += spatial_kernel * temporal_kernel;
			}
		...
		// Different edge case
		// Compute distances from q to c and d
		dist_q_c = min(cur_l.dist_n1 + our_model.sp_node_vec_node_a[node_index_pi_c].cur_sp_value,
		               cur_l.dist_n2 + our_model.sp_node_vec_node_b[node_index_pi_c].cur_sp_value);
		dist_q_d = min(cur_l.dist_n1 + our_model.sp_node_vec_node_a[node_index_pi_d].cur_sp_value,
		               cur_l.dist_n2 + our_model.sp_node_vec_node_b[node_index_pi_d].cur_sp_value);
		
		// If both endpoints are too far, skip this edge
		if (dist_q_c > our_model.bandwidth && dist_q_d > our_model.bandwidth)
			return 0;
		
		// For points on different edges in spatiotemporal mode with ADA,
		// we iterate through points and filter by time window
		for (int i = 0; i < (int)PS.size(); i++)
		{
			// Check if point is in time window
			if (!is_point_in_time_window(our_model, PS[i].time, cur_time))
			...
			// Calculate spatial distance
			dist_q_p = min(dist_q_c + PS[i].dist_n1, dist_q_d + PS[i].dist_n2);
	...
	// Basic spatiotemporal algorithm (method 1 or 2)
	if (our_model.method == 1 || our_model.method == 2)
	{
		// Same edge case
		if (cur_l.edge_index == edge_index)
		...
		for (int i = 0; i < (int)PS.size(); i++)
		{
			...
			// Calculate spatial distance using Dijkstra results
			if (our_model.method == 1)
				dist_q_p = min(our_model.sp_node_vec[node_index_pi_c].cur_sp_value + PS[i].dist_n1,
				               our_model.sp_node_vec[node_index_pi_d].cur_sp_value + PS[i].dist_n2);
			
			if (our_model.method == 2)
				dist_q_p = min(cur_l.dist_n1 + min(our_model.sp_node_vec_node_a[node_index_pi_c].cur_sp_value + PS[i].dist_n1, 
				                                    our_model.sp_node_vec_node_a[node_index_pi_d].cur_sp_value + PS[i].dist_n2),
				               cur_l.dist_n2 + min(our_model.sp_node_vec_node_b[node_index_pi_c].cur_sp_value + PS[i].dist_n1, 
				                                    our_model.sp_node_vec_node_b[node_index_pi_d].cur_sp_value + PS[i].dist_n2));
...
void NKDV_spatiotemporal(model& our_model)
{
	...
	for (int e = 0; e < our_model.m; e++)
	{
		edge_KA_value = edge_KAF_spatiotemporal(our_model, e, cur_time);
		cur_l.KDE_value += edge_KA_value;
	}
}
