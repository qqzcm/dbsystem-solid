...
public class IRTreeTest {
    ...
    public IRTree irTree;

    public InvertedIndex<RelevantObject> defaultLeafInvertedIndex;
    ...
    public void testIRTree() {
        ...
        defaultLeafInvertedIndex = new DefaultLeafInvertedIndex(relevantObjectService);

        Node<String, Geometry> root = irTree.getRTree().root().get();

        Map<String, List<IRTree.NodePair>> nonLeafInvFile = irTree.getNonLeafInvFile(root);

        // 声明为 final 或者 effectively final 的 TreeMap 变量
        final TreeMap<String, Integer> treeMap = new TreeMap<>();
        // 创建 TreeMap 并传入自定义的比较器（按照 value 的大小排序）
        TreeMap<String, Integer> sortedTreeMap = new TreeMap<>(Comparator.comparing(treeMap::get));
        for (String x : nonLeafInvFile.keySet()) {
            treeMap.put(x, defaultLeafInvertedIndex.getValues(x).size());
        }
